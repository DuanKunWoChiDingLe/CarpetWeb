...
// 消息回调映射，消息体类型未知，使用 unknown 让调用者自行断言
const subscriptions: Map<string, (msg: unknown) => void> = new Map()

// 获取基础 URL（用于 WebSocket 端点）
const baseURL = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080'

export const connectWebSocket = (onConnect?: () => void, onError?: (error: IFrame | Event) => void) => {
  if (stompClient && isConnected) {
    onConnect?.()
    return
  }
...
export const subscribe = (destination: string, callback: (msg: unknown) => void) => {
  console.log('subscribe called with destination:', destination)
  subscriptions.set(destination, callback)
  if (isConnected && stompClient) {
    console.log('立即订阅', destination)
